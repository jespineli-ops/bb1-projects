 ...
 * 08/26/2026     Jared Espineli        Sourced real data via data_lib, matching the PDF's columns, with amounts fixed to 2 decimal places
 ...
 * 08/28/2026     Jared Espineli        Rebuilt around a raw, one-row-per-Unit column set driven by As of Date, with new lease/tenant columns and a merged title/printed row
 * 09/03/2026     Jared Espineli        Renamed several headers and switched key columns to display text instead of internal ids
 * 09/04/2026     Jared Espineli        Printed timestamp now reflects the account's timezone, Rent Esc% exports as a trimmed percentage, and Units linked to multiple Buildings export one row per Building
 *
...
define(['N/file', './bb1_qpg_tschd_report_lib_helper', './bb1_qpg_tschd_report_data_lib'],
    ...
    (file, helperLib, dataLib) => {
        ...
        // South Africa Standard Time is a flat UTC+2 year-round.
        const SAST_OFFSET_MINUTES = 120;

        const toSouthAfricanTime = (date) => new Date(date.getTime() + (SAST_OFFSET_MINUTES * 60000));

        // CSV column headers, in the CSV's own raw-data column order.
        const COLUMNS = [
        ...
        // Money columns exported as plain 2-decimal numbers.
        ...
        // Index of the Rent Esc% column, exported as a trimmed percentage rather than a raw fraction.
        const RENT_ESC_COLUMN_INDEX = COLUMNS.indexOf('Rent Esc%');

        const formatDataRow = (values) => values.map((value, index) => {
            if (value === null || value === undefined || value === '') return value;
            if (index === RENT_ESC_COLUMN_INDEX) {
                const num = Number(value);
                return isNaN(num) ? value : `${parseFloat((num * 100).toFixed(2))}%`;
            }
            if (!AMOUNT_COLUMN_INDEXES.has(index)) return value;
            const num = Number(value);
            return isNaN(num) ? value : num.toFixed(2);
        });

        // Column where the title/printed text is centered above the table.
        ...
        LIB_FX.buildCsv = (params) => {
            ...
            const printedText = helperLib.LIB_FX.formatPrintedTimestamp(toSouthAfricanTime(new Date()));
            ...
            // Title row, blank row, column headers, then one data row per Unit.
            const rows = [
                titleRow,
                [],
                COLUMNS,
